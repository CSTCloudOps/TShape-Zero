...
The main ledger contains 44 executable configurations. N marks no-fit residual controls; F marks released foundation checkpoints executed without target fitting; S marks our strict synthetic-only checkpoints; A marks source-adaptation diagnostics; X marks full-coverage family adapters; T marks a target-context diagnostic; and B marks ablations. Only N/F/S rows participate in the red/blue deployment ranking. This distinction is essential: an adapter tests a scoring family under the common interface but is not represented as the original authors' checkpoint.

The N group contains persistence, rolling mean, rolling median, spectral residual, and rolling MAD. The F group executes Chronos-Bolt-Tiny, Timer-base-84M, TimesFM-2.5-200M, and Sundial-base-128M directly; their preceding-context forecasts are converted to squared residual scores. Context/horizon pairs are 64/64, 384/96, 384/96, and 720/720. Official TimeMoE-50M exceeded the predeclared ten-minute optional-checkpoint screen and is not reported partially. A separately marked TimeMoE family adapter remains an X row. The ledger also retains every family represented in the original TShape comparison, including SubLOF, SAND, Matrix Profile, AR, LSTMAD, AE, EncDecAD, SRCNN, Anomaly Transformer, TFAD, TranAD, Donut, FCVAE, TimesNet, OFA, FITS, and KAN-AD.
...
Table~\ref{tab:all-easytsad} is the primary result. It deliberately keeps all completed methods and all six targets visible while separating their data-access contracts. The red and blue values answer a narrow, reproducible question: among methods that require no target fitting, which score streams are strongest under the common EasyTSAD protocol? Adapter rows broaden the diagnostic landscape but cannot win that ranking.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_strict_overall.pdf}
  \caption{Macro performance of directly deployable N/F/S methods. Each segment connects a method's \efone{} (square) and \pfone{} (circle); the complete per-dataset ledger and access status appear in Table~\ref{tab:all-easytsad}.}
  \label{fig:overall}
\end{figure}
...
The per-dataset cells expose complementary regimes. \method{} ranks first among N/F/S methods on Yahoo for both metrics, while the Pattern-only checkpoint ranks first on NAB. TimesFM is strongest on several AIOPS, TODS, and WSD cells; spectral residual leads UCR \pfone{}, whereas persistence leads UCR \efone{}. This heterogeneity is precisely why an out-of-the-box product must retain channel evidence and why average-only claims would be unsafe.
