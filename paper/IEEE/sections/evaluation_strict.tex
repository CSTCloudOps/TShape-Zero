...
The residual controls are persistence, rolling mean, rolling median, spectral residual, and rolling MAD. The focal frozen-checkpoint group executes Chronos-Bolt-Tiny, Timer-base-84M, and Sundial-base-128M directly; their preceding-context forecasts are converted to squared residual scores. Context/horizon pairs are 64/64, 384/96, and 720/720. Official TimeMoE-50M exceeded the predeclared ten-minute optional-checkpoint screen and is not reported partially. A separately identified TimeMoE family adapter remains in the table. The ledger also retains every family represented in the original TShape comparison, including SubLOF, SAND, Matrix Profile, AR, LSTMAD, AE, EncDecAD, SRCNN, Anomaly Transformer, TFAD, TranAD, Donut, FCVAE, TimesNet, OFA, FITS, and KAN-AD.
...
\input{generated/easytsad_main_table.tex}
...
Fig.~\ref{fig:overall} makes the central comparison readable without hiding the long table. Pattern-only TShape-Zero reaches \PatternMacroPoint{}/\PatternMacroEvent{} macro \pfone{}/\efone{}. Adding the guard raises the result to \FinalMacroPoint{}/\FinalMacroEvent{}, a gain of 0.044/0.032. Timer is the strongest focal official checkpoint at 0.835/0.710; \method{} is higher by 0.010 \pfone{} and 0.023 \efone{}. Rolling median remains a serious deployment control at 0.839/0.734 and is marginally higher than \method{} on macro \efone{}. Thus, the result supports a compact and inspectable TShape upgrade, not universal superiority.

The per-dataset cells expose complementary regimes. Among the ranked frozen/strict checkpoints, \method{} leads both metrics on TODS and Yahoo and leads AIOPS \pfone{}; the Pattern-only checkpoint leads NAB. Timer leads both UCR metrics, while Sundial leads AIOPS \efone{} and both WSD metrics. Some adapter rows obtain larger individual cells under different packaging or adaptation contracts, which is why their provenance remains explicit in the public ledger rather than being conflated with official frozen-checkpoint evidence. This heterogeneity is precisely why an out-of-the-box product must retain channel evidence and why average-only claims would be unsafe.
...
\begin{figure}[t]
  ...
  \caption{Pattern Bank optimization from 10\% to 100\%. (a) Training trajectories with a narrow, symmetric cross-size mean $\pm$ SEM band. (b) Best training MSE with a symmetric band showing $\pm$1 SD over the final five epochs. These bands describe optimization across bank sizes and late epochs, not training-seed uncertainty; every size uses the same generator mixture and seed.}
...
\begin{figure}[t]
  ...
  \caption{Pattern Bank size sweep on all six datasets, arranged as two rows of three panels. Dotted curves are the measured neural Pattern-only channel; solid curves are the same checkpoints after the fixed Residual Guard, with symmetric across-series mean $\pm$ SEM bands. Every marker is a full-series \pfone{} or \efone{} evaluation; panel-specific ranges expose within-target scaling and saturation without implying cross-panel scale equality.}
...
\input{generated/easytsad_paired_table.tex}
...
\begin{figure}[t]
  ...
  \caption{EasyTSAD channel attribution over paired valid series. (a) Stacked bars identify the strongest individual evidence channel; diamonds show how often final fusion beats the exactly matched guard. (b) Each jittered point is one KPI's final-fusion gain over that guard, colored by its strongest individual channel; diamonds and whiskers report the dataset mean and 95\% bootstrap CI.}
  \label{fig:channel-attribution}
\end{figure}

Fig.~\ref{fig:channel-attribution} asks a stricter question than a favorable case study: can one expose where the neural channel adds value? Panel (a) compares Pattern-only TShape with median, spectral, and combined-guard evidence on every paired valid series, then reports the final fusion win rate against that same guard. Panel (b) preserves the per-series distribution instead of reducing it to a dataset average. Its ordinate is the change in the paired mean of \pfone{} and \efone{} from the exactly matched guard to final fusion; point color links each outcome back to the strongest standalone channel in panel (a). Positive and negative observations coexist, while the dataset summaries are positive for AIOPS, NAB, WSD, and Yahoo and slightly negative for TODS and UCR. This open-box evidence is more actionable than an opaque learned router: operators can inspect whether an alarm was supported by Pattern, residual evidence, or both.
...
Fig.~\ref{fig:effectiveness} combines effectiveness with release cost. The \method{} checkpoint is 0.138 MiB, compared with 33.0 MiB for Chronos, 321.0 MiB for Timer, and 489.6 MiB for Sundial. Relative to Timer, the strongest focal official checkpoint in macro F1, \method{} is higher by 0.010 \pfone{} and 0.023 \efone{} while being over 2,300 times smaller. The final Pattern model trains in 128 seconds; dense stride-one inference over all 834 series and 15.1 million aligned points takes 291 seconds on the study MPS device. Footprint is not latency, but it directly affects artifact transfer, cold start, versioning, and edge placement. Runtime metadata remain available separately so the bubble plot does not pretend that heterogeneous foundation-model hardware timings are architecture-only effects.
